...
card_G, aro = card_graph(
	obo_file=DATA_DIR + '/card/card-ontology/aro.obo',
  json_file=DATA_DIR + '/card/card-data/card.json',
	card_map_file=BASE_DIR + '/map_tsv/card_map.tsv', 
	acc=acc, 
	color=group_colors.get('card', 'black')
	)
...
### merge nodes with same predecesor and sucesor nodes
...
for node in net.nodes:
  ...
  node['label'] = node['label']  # Show label on the node
  node['color'] = G.nodes[node['id']].get('color', 'black')
